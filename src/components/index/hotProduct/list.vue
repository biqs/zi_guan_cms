<template>
 ...
 <table class="table m-b-none footable-loaded footable default" ui-jq="footable" data-filter="#filter" data-page-size="5">
          ...
          <!--       <td>    
                    ...
                    <td class="text-center" >{{item.productType  | getName}}</td>
                    <td  >
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods:{
      loadData(){
        this.axios.get(HotProduct.query(this.currentPage, PAGE_SIZE)).then((response)=>{
      ...
      editItem(item){
        ...
        this.$confirm('此操作将永久删除该产品, 是否继续?', '提示', {
                ...
                this.axios.get(HotProduct.delete(item.id))
                .then((response)=>{
      ...
      order(item,index,status){  //status : 0(上移)： 1： 下移
        ...
        replaceObject = this.hotItems[index + 1];
          ...
          this.axios.post(HotProduct.order,{
